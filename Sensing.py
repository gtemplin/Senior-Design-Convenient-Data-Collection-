...
def data_to_csv_file(file_name, data):
    with open(Curpath+file_name, 'a', encoding='UTF8', newline='') as f:
        writer = csv.writer(f)
        # write multiple rows
...
def Prepare_and_Send_Message0(systemData, IPAddress):
    ...
    msg_to_file(str(Curpath)+"/FormattedSystemData.txt", companyInfo)
    ff = open(str(Curpath)+"/CommunicationFlag.txt", "w+")
    ff.close()
...
def Prepare_and_Send_Message1(systemData):
    for sensor in systemData["SensingPorts"]:
        ...
        msg_to_file(str(Curpath)+"/FormattedSystemData.txt", sensorMsg)
        ff = open(str(Curpath)+"/CommunicationFlag.txt", "w+")
        ff.close()
...
def Prepare_and_Send_Message3(systemData):
    for switch in systemData["SwitchingPorts"]:
        ...
        msg_to_file(str(Curpath)+"/FormattedSystemData.txt", switchMsg)
        ff = open(str(Curpath)+"/CommunicationFlag.txt", "w+")
        ff.close()
...
def Prepare_and_Send_Message2(systemData, previous_database_send_time):
    if update_time_flag(previous_database_send_time, int(systemData["DatabaseSendIntervalInSeconds"])):
        ...
        if( packed_msg_processed!=""):
            ...
            msg_to_file(str(Curpath)+"/FormattedSystemData.txt", packed_msg_processed)
            ff = open(str(Curpath) + "/CommunicationFlag.txt", "w+")
            ff.close()
...
if __name__ == "__main__":
    configDebug = str(Curpath)+"/configDebug.json"
    flag = json_to_dict(configDebug, None)

    configCustomer = str(Curpath)+"/configCustomer.json"
